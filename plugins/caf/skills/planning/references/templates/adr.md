# ADR NNN: [Short title describing the decision]

## Status

[Proposed | Accepted | Superseded by ADR-XXX | Deprecated]

## Context

[Describe the context and problem statement. What decision needs to be made and why?]

[Include relevant background information:]
- What is driving this decision?
- What constraints exist (technical, business, team, timeline)?
- What questions need to be answered?
- What are the key considerations?

[Keep this section focused on the problem, not the solution]

## Decision

[State the decision clearly and concisely]

[For domain decisions, describe:]
- Core entities and their relationships
- Key domain concepts and rules
- How entities fit together

[For technical decisions, describe:]
- What technology/approach was chosen
- How it will be used
- Any important configuration or setup details

[For data architecture decisions, describe:]
- Schema structure or data pattern
- Key tables/collections and relationships
- Important indexes or performance considerations

## Consequences

**Positive:**
- [Benefit 1]
- [Benefit 2]
- [Benefit 3]
- [Benefit 4]
- [Benefit 5]

**Negative:**
- [Cost, limitation, or risk 1]
- [Cost, limitation, or risk 2]
- [Cost, limitation, or risk 3]
- [Cost, limitation, or risk 4]
- [Cost, limitation, or risk 5]

[Be honest about trade-offs. Every decision has downsides.]

## Alternatives considered

### Option 1: [Alternative name]

[Brief description of this alternative]

**Pros:**
- [Advantage 1]
- [Advantage 2]
- [Advantage 3]

**Cons:**
- [Disadvantage 1]
- [Disadvantage 2]
- [Disadvantage 3]

**Why not chosen:**
[Explain the key reason this wasn't selected]

### Option 2: [Alternative name]

[Brief description of this alternative]

**Pros:**
- [Advantage 1]
- [Advantage 2]
- [Advantage 3]

**Cons:**
- [Disadvantage 1]
- [Disadvantage 2]
- [Disadvantage 3]

**Why not chosen:**
[Explain the key reason this wasn't selected]

[Add more alternatives as needed - aim for 2-4 total options including the chosen one]
